...
import type { ProfileConfiguration, ResponseSchema } from './profile.js';
...
/**
 * The result of a Generate Summary request.
 */
export interface Summary {
  /** The AI-generated summary text. */
  summary?: string;
  /** Structured response map when a response schema was provided. Keys and value types are determined by the schema. */
  response?: Record<string, unknown>;
  /** Echoes the structured response schema used, enabling clients to validate/interpret the dynamic response map. */
...
export default class SessionSdk extends Client {
   ...
   */
  async summary(sessionId: string, profileId: string, endTimestamp?: string): Promise<Summary> {
    const id = this.parseId(sessionId);
  ...
  /**
   * Return a Generative AI summarization of the specified session using a combination of a saved summarization profile and local overrides. This allows you to use a saved profile as a base configuration while customizing certain aspects of the prompting instructions or context configuration for a specific summarization request.
   * @param sessionId The UTF-8 encoded ID of the session for which to generate context data. To retrieve the Session ID you may use the appropriate Get Session Details API for Web, iOS, or Android. As Session IDs include : separators the ID must be URL encoded using the %3A substitution.
   * @param configuration A partial profile configuration object containing only the fields to override from the saved profile. This can include any subset of the ProfileConfiguration fields, allowing you to customize prompting instructions, context configuration, and slice settings on a per-request basis.
   * @param profileId Optional ID of the saved summarization profile to use as a base configuration. If omitted, the local configuration will be used in its entirety without referencing a saved profile.
   * @returns An object containing the `summary` string generated by the AI model.
   */
  async summaryWithOverrides(sessionId: string, configuration: ProfileConfiguration, profileId?: string): Promise<Summary> {
    const id = this.parseId(sessionId);
    return this.POST<ProfileConfiguration & { config_profile?: string }, Summary>(`/v2/sessions/${encodeURIComponent(id)}/summary`, {
