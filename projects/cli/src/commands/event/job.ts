...
export default class EventJobCommand extends Command {
  static args = {
    jobId: Args.string({ required: true, description: 'The batch import job ID.' }),
  };

  static description = `Get the status of a batch event import job.
...
For more information, see https://developer.fullstory.com/server/events/get-batch-events-import-job/`;
  ...
  static examples = [
    { command: '<%= config.bin %> event:job abc-123', description: 'Check the status of a batch import job.' },
  ];
  ...
  static summary = 'Get batch event import job status.';

  async run() {
    const { args: { jobId } } = await this.parse(EventJobCommand);
    const { Event } = this.Fullstory;

    const job = await Event.job(jobId);
